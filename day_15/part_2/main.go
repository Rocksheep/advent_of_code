...
var oxygenMap map[Point]int = map[Point]int{}
	...
	for processor.Stopped == false {
		...
		if processor.Waiting == true {
			...
			if objectFound && robotPosition.x == 0 && robotPosition.y == 0 {
				spreadOxygen(oxygenSystem, 0)
				highestDuration := 0
				for _, value := range oxygenMap {
					if value > highestDuration {
						highestDuration = value
					}
				}
				fmt.Println(highestDuration)
				break
			}
		}
	}
...
func spreadOxygen(p Point, distance int) {
	if _, ok := oxygenMap[p]; ok == false {
		time.Sleep(1 * time.Millisecond)

		if val, ok := tiles[p]; ok {
			if val == 1 {
				mutex.Lock()
				oxygenMap[p] = distance
				mutex.Unlock()
				spreadOxygen(Point{p.x, p.y - 1}, distance+1)
				spreadOxygen(Point{p.x, p.y + 1}, distance+1)
				spreadOxygen(Point{p.x - 1, p.y}, distance+1)
				spreadOxygen(Point{p.x + 1, p.y}, distance+1)
			}
		}
	}
