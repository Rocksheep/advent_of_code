...
import (
	"advent_of_code/day_17/part_2/opcode"
	"fmt"
	"image/color"
	"log"
	"strconv"
...
func startIntMachine() {
	...
	fmt.Println(sumAlignmentParameters())
	path := makePath([]string{})

	functions := map[string]string{}
	_, compressedPath := findPatterns(0, functions, path)

	mainFunc := strings.Join(compressedPath, ",") + "\n"
	funcA := functions["A"] + "\n"
	funcB := functions["B"] + "\n"
	funcC := functions["C"] + "\n"
	rawInput := mainFunc + funcA + funcB + funcC + "n\n"
	formattedInput := make([]int64, len(rawInput))
	for i, c := range rawInput {
		formattedInput[i] = int64(c)
	}

	opCode[0] = 2
	processor := opcode.New(opCode)
	processor.SetInput(formattedInput)
...
func isIntersection(point Point) bool {
	return isScaffold(Point{point.x, point.y - 1}) &&
		isScaffold(Point{point.x + 1, point.y}) &&
		isScaffold(Point{point.x, point.y + 1}) &&
		isScaffold(Point{point.x - 1, point.y})
}

func isScaffold(point Point) bool {
	if val, ok := tiles[point]; ok {
		return val == '#'
	}
	return false
}
...
func sumAlignmentParameters() int {
	...
	for _, point := range intersections {
		sum += alignmentParameters(point)
	}
	return sum
}

func alignmentParameters(point Point) int {
...
func makePath(path []string) []string {
	var dPoint Point
	direction := 'L'
	if isScaffold(Point{robot.point.x, robot.point.y - 1}) && robot.direction != 3 {
		if robot.direction == 4 {
			direction = 'R'
		}
		dPoint = Point{0, -1}
		robot.direction = 1
	} else if isScaffold(Point{robot.point.x + 1, robot.point.y}) && robot.direction != 4 {
		if robot.direction == 1 {
			direction = 'R'
		}
		dPoint = Point{1, 0}
		robot.direction = 2
	} else if isScaffold(Point{robot.point.x, robot.point.y + 1}) && robot.direction != 1 {
		if robot.direction == 2 {
			direction = 'R'
		}
		dPoint = Point{0, 1}
		robot.direction = 3
	} else if isScaffold(Point{robot.point.x - 1, robot.point.y}) && robot.direction != 2 {
	...
	mutex.Lock()
	distance := 0
	for isScaffold(Point{robot.point.x + dPoint.x, robot.point.y + dPoint.y}) {
		robot.point.x += dPoint.x
		robot.point.y += dPoint.y
		// time.Sleep(100 * time.Millisecond)
		distance++
	}
	mutex.Unlock()

	path = append(path, string(direction)+","+strconv.Itoa(distance))

	return makePath(path)
}

func findPatterns(depth int, chunks map[string]string, path []string) (bool, []string) {
	if depth >= 3 {
		flatPath := flattenPath(path, ".")
		if strings.Contains(flatPath, "L") || strings.Contains(flatPath, "R") {
			return false, path
		}
	...
	for i := range path {
		for j := i + 1; j < len(path); j++ {
			...
			key := string(rune(depth + 'A'))
			numMatches, restPath := findMatches(key, part, path)
			if numMatches > 1 {
				// fmt.Println(part, numMatches)
				chunks[key] = flattenPath(part, ",")
				result, path := findPatterns(depth+1, chunks, restPath)
				if result == true {
					return true, path
				}
				delete(chunks, key)
...
func findMatches(key string, a []string, b []string) (int, []string) {
	flatA := flattenPath(a, ".")

	if len(flatA) > 20 || strings.Contains(flatA, "A") || strings.Contains(flatA, "B") || strings.Contains(flatA, "C") {
		return 0, b
	}

	flatB := flattenPath(b, ".")
	count := strings.Count(flatB, flatA)
	result := strings.Replace(flatB, flatA, key, -1)

	return count, strings.Split(result, ".")
}

func flattenPath(path []string, delimiter string) string {
	flatPath := ""
	for i, s := range path {
		flatPath += s
		if i != len(path)-1 {
...
func update(screen *ebiten.Image) error {
	...
	for point, id := range tiles {
		...
		opts.GeoM.Translate(float64(point.x*8), float64(point.y*8))
	...
	opts := &ebiten.DrawImageOptions{}
	square.Fill(color.NRGBA{0xFF, 0xFF, 0x00, 0xFF})
	opts.GeoM.Translate(float64(robot.point.x*8), float64(robot.point.y*8))
	screen.DrawImage(square, opts)
